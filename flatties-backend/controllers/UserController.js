...
const UserController = {
  ...
  addToWatchlist: async (req, res) => {
    try {
      ...
      const propertyId = req.body.propertyId; // Assuming you pass the propertyId in the request body
      ...
      // Find the watching list for the user
      const watchingList = await WatchingListModel.findById(user.watchingList);
      if (!watchingList) {
        return res.status(404).json({ message: "Watching list not found" });
      }

      // Add the property to the watching list
      watchingList.properties.push(propertyId);
      const updatedWatchingList = await watchingList.save();

      res.status(200).json({ success: true, user, updatedWatchingList });
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: "Server Error" });
    }
  ...
  removeFromWatchlist: async (req, res) => {
    try {
      ...
      watchingList.properties = watchingList.properties.filter(
        (id) => id !== propertyId
      );
      const updatedWatchingList = await watchingList.save();
